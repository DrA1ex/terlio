...
Code, diff and command blocks use `code-safe` Unicode rendering by default. Tool-result blocks use normal Unicode unless `contentKind` identifies code, a log or security-sensitive output. ZWJ/ZWNJ remain intact even in `code-safe`; set `unicodeSecurity` explicitly when a block needs a different display policy.
