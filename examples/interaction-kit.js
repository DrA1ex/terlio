...
function renderNavPane({ state, theme, height }) {
  ...
  return Column({ height },
    WorkspacePane({
      ...
      height: listHeight,
      theme,
      children: [SelectList({ title: 'Entries', items: SHOWCASES, selectedIndex: state.selectedShowcaseIndex, windowSize: Math.max(2, Math.floor((listHeight - 5) / 2)), getLabel: (item) => item.title, getDescription: (item) => item.category, wrapItems: true, rowLines: 2, reserveItemLines: true, theme })],
...
const SHOWCASES = [
  ...
  { id: 'workspace-shell-anatomy', title: 'Workspace Shell Anatomy', category: 'Layout', summary: 'Shows a recommended app shell with local pane focus, scrollable regions, command entry, history, and autocomplete.', components: ['WorkspaceShell', 'InputEditor', 'WorkspaceCommandBar', 'ScrollPane'], controls: [{ key: 'Tab', action: 'focus/complete' }, { key: '[/]', action: 'switch tab' }, { key: 'text', action: 'type command' }, { key: 'Enter', action: 'submit command' }, { key: '↑/↓ Pg', action: 'history/scroll' }], createInitialState: () => ({ focusIndex: 0, tabIndex: 0, command: new InputEditor(''), history: ['test ui --watch', 'explain renderer diff', 'reset workspace'], historyIndex: null, activity: ['workspace opened', 'main pane focused', 'press Tab twice to reach the command bar', 'try typing “de”, “te”, “ex”, or “re” and press Tab'], mainScroll: createScrollState({ totalRows: 18, visibleRows: 8, sticky: false }), activityScroll: createScrollState({ totalRows: 4, visibleRows: 8, sticky: true }) }), render: ({ state, theme, width }) => {
    ...
    return WorkspaceShell({
      ...
      tabs,
      activeTab,
      tabHint: '[/] tab · Tab local focus · command Tab autocomplete · Enter submit',
      main: Row({ gap: 2, widths: [Math.max(34, Math.floor(width * 0.42)), Math.max(34, Math.floor(width * 0.34))] },
        WorkspacePane({ title: ' MAIN ', active: focus === 'main', theme, children: [ScrollPane({ title: `${tabs[state.tabIndex].label} content`, lines: mainLines, width: Math.max(30, Math.floor(width * 0.42)), height: 10, scroll: state.mainScroll.scroll, footer: true })] }),
        WorkspacePane({ title: ' ACTIVITY ', active: focus === 'activity', theme, children: [ScrollPane({ title: 'Command activity', lines: activityLines, width: Math.max(30, Math.floor(width * 0.34)), height: 10, scroll: state.activityScroll.scroll, footer: true })] }),
      ),
  ...
  { id: 'structured-assistant-blocks', title: 'Structured Assistant Blocks', category: 'AI blocks', summary: 'Shows scrollable chat-oriented structured rendering with text, code, diff, command, warning, and tool result blocks.', components: ['createBlock', 'renderBlocksLines', 'ScrollPane', 'transcript roles'], controls: [{ key: '[/]', action: 'scenario' }, { key: '↑/↓ Pg', action: 'scroll' }, { key: 'c', action: 'copy toast' }], createInitialState: () => ({ scenario: 0, generation: 1, scroll: createScrollState({ totalRows: 0, visibleRows: 13, sticky: false }) }), render: ({ state, theme, width }) => {
    ...
    const transcript = ScrollPane({ title: ` Assistant response · ${scenario.title} `, lines: transcriptLines, width: transcriptWidth, height: 17, scroll: state.scroll.scroll, footer: true });
    return Column({ gap: 1 },
      SectionTabs({ tabs: blockScenarios().map((item, index) => ({ id: String(index), label: item.title })), active: String(state.scenario), theme }),
  ...
  { id: 'focus-and-modes', title: 'Focus and Modes', category: 'Navigation', summary: 'Shows local focus ownership, selectable focus targets, disabled targets, and a mode stack that restores focus on pop.', components: ['FocusManager', 'ModeManager', 'SelectList', 'ActionRegistry'], controls: [{ key: '↑/↓', action: 'select target' }, { key: 'Enter', action: 'focus selected' }, { key: 'Tab', action: 'local next' }, { key: 'Shift+Tab', action: 'local previous' }, { key: 'm/h/p', action: 'push modal/help/pop' }, { key: 'd', action: 'toggle selected' }], createInitialState: () => ({ focus: new FocusManager(['nav', 'preview', 'command', 'modal', 'help']), modes: new ModeManager('root'), list: createListState({ items: ['nav', 'preview', 'command', 'modal', 'help'], windowSize: 6, skipDisabled: false }), owner: 'local preview' }), render: ({ state, theme }) => {
    ...
    return Column({ gap: 1 },
      Row({ gap: 2, distribute: true },
        SelectList({ title: 'Focus targets', items: state.list.items, selectedIndex: state.list.selectedIndex, windowSize: 6, getLabel: (id) => `${state.focus.current() === id ? '●' : ' '} ${id}`, getDescription: (id) => `${state.focus.isEnabled(id) ? 'enabled' : 'disabled'}${id === selectedId ? ' · selected' : ''}`, theme }),
