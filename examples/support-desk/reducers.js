...
export function tickSupportDesk(state) {
  ...
  if (state.frame > 0 && state.frame % 32 === 0) {
    injectLiveSupportEvent(state);
  }
}
...
function injectLiveSupportEvent(state) {
  ...
  const templates = [
    ...
    { queue: 'Platform', priority: 'High', subject: 'Webhook endpoint receives duplicate retries', tag: 'webhook', customer: 'Amal Singh', account: 'Atlas Grid' },
    { queue: 'Product', priority: 'Low', subject: 'CSV export column order changed', tag: 'export', customer: 'Julia Novak', account: 'Silverline Ops' },
  ];
  const item = templates[sequence % templates.length];
  const id = `TCK-${1200 + sequence}`;
  const ticket = {
    id,
    ...
    assignee: '',
    tags: [item.tag, item.queue.toLowerCase()],
    slaMinutes: item.priority === 'High' ? 18 : 95,
    originalSlaMinutes: 180,
    customer: { name: item.account, contact: item.customer, plan: item.priority === 'High' ? 'Business' : 'Team', health: item.priority === 'High' ? 'Watch' : 'Healthy', email: `${item.customer.toLowerCase().replace(/\s+/g, '.')}@example.test`, timezone: 'UTC+1', mrr: item.priority === 'High' ? '$1.9k' : '$740' },
    summary: `Live mock ticket generated by the support event stream. Queue=${item.queue}, priority=${item.priority}.`,
    messages: [{ id: `msg_${Date.now()}_${sequence}`, role: 'customer', author: item.customer, body: item.subject, time: new Date().toISOString() }],
    notes: [],
    linkedTickets: [],
    customFields: { productArea: item.queue, impact: item.priority === 'High' ? 'Multiple users affected' : 'Single workspace', source: 'Live event' },
    timeline: [createTimelineEvent({ type: 'customer_message', actor: item.customer, text: `live ticket created: ${item.subject}` })],
    activity: ['Created from live mock feed', `Queue ${item.queue}`, `Tag ${item.tag}`],
  };
