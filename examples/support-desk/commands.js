...
export const SUPPORT_COMMANDS = [
  ...
  command('customer', 'customer', 'Navigation', 'Switch to customer tab.', ({ state }) => { state.activeTab = 'customer'; state.toasts.show('Customer profile selected.', 'info'); return { ok: true, action: 'customer' }; }, [], ['/customer']),
  command('timeline', 'timeline', 'Navigation', 'Switch to timeline tab.', ({ state }) => { state.activeTab = 'activity'; state.toasts.show('Activity timeline selected.', 'info'); return { ok: true, action: 'timeline' }; }, [], ['/timeline']),
  command('goto', 'goto <inbox|ticket|reply|customer|activity>', 'Navigation', 'Switch primary product tab.', ({ state, parsed }) => { const tab = String(parsed.args[0] ?? '').toLowerCase(); if (!['inbox','ticket','reply','customer','activity'].includes(tab)) { state.toasts.show(`Unknown tab: ${tab}`, 'error'); return { ok: false, reason: 'bad-tab' }; } state.activeTab = tab; state.toasts.show(`Opened ${tab} tab.`, 'info'); return { ok: true, action: 'goto' }; }, ['go'], ['/goto reply']),
  ...
  command('activity', 'activity <next|prev>', 'Navigation', 'Switch to Activity tab or move its pagination.', ({ state, parsed }) => { state.activeTab = 'activity'; const dir = String(parsed.args[0] || '').toLowerCase(); if (dir === 'next') state.activityPage = (state.activityPage || 0) + 1; if (dir === 'prev') state.activityPage = Math.max(0, (state.activityPage || 0) - 1); state.focus = 'activity'; state.toasts.show('Activity tab selected.', 'info'); return { ok: true, action: 'activity' }; }, [], ['/activity']),
