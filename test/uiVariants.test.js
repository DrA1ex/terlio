...
test('feedback components render every state, tone and optional surface', () => {
  ...
  assert.match(stripAnsi(render(ProgressBar({ value: 1, total: 16, width: 8, variant: 'block' }), 20, 1)), /\[▌       \] 6%/);
  assert.match(stripAnsi(render(ProgressBar({ value: 1, total: 16, width: 8, variant: 'line' }), 20, 1)), /\[▌───────\] 6%/);
  assert.match(stripAnsi(render(ProgressBar({ value: 50, total: 100, width: 8, variant: 'squares' }), 20, 1)), /\[■■■■□□□□\] 50%/);
  assert.match(stripAnsi(render(ProgressBar({ value: 0, total: 100, width: 8, variant: 'inset' }), 20, 1)), /▏▁▁▁▁▁▁▁▁▕ 0%/);
  assert.match(stripAnsi(render(ProgressBar({ value: 1, total: 16, width: 8, variant: 'inset' }), 20, 1)), /▏▌▁▁▁▁▁▁▁▕ 6%/);
  assert.match(stripAnsi(render(ProgressBar({ value: 100, total: 100, width: 8, variant: 'inset' }), 20, 1)), /▏████████▕ 100%/);
